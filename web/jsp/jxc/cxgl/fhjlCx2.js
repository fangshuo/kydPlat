Ext.onReady(function() {
	...
	var fieldset = new Ext.form.FieldSet({
		...
		// //
		// proxy: new
		// Ext.data.HttpProxy({url:'ComboboxAction!queryPpgg.action'}),
		// reader: new Ext.data.JsonReader({
		// root:'combobox',
		// fields:['value','text']}),
		// remoteSort: true }) ,
	...
	Ext.getCmp('zkhmcCombo').on('beforequery', function(e) {
		if (!e.forceAll) {
			pymValue = e.query;
			Ext.getCmp('zkhmcCombo').getStore().reload({
				callback : function(records, options, success) {
					if (Ext.getCmp('zkhmcCombo').getStore().getCount() > 0
							&& (!pymValue == "")) {
						var firstValue = Ext.getCmp('zkhmcCombo').getStore()
								.getRange()[1].data.value;// 这种方法可以获得第一项的值
...
//										}
	...
	var form = new Ext.form.FormPanel({
				...
				submit : function() {
					form.getForm().getEl().dom.target = '_blank', form
							.getForm().getEl().dom.action = 'fhjlCxQuery2!dcExcel.action';
	...
	var splbRecord = Ext.data.Record.create([{
			...
			// {name: 'fldj' ,mapping:'fldj' ,type: 'float'},
			// {name: 'je' ,mapping:'je' ,type: 'float'}
			]);

	var store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
					url : 'fhjlCxQuery2!queryData.action',
	...
	var grid = new Ext.grid.EditorGridPanel({
				...
				cm : columns,
				// 每次只能选择一行new
				// Ext.grid.RowSelectionModel({singleSelect:true}),，如果sm：checkbox
				// 则可以多选
	...
	store.on('beforeload', function() {
				...
				Ext.apply(this.baseParams, {
							fhsjq : Ext.getCmp('fhsjq').getValue()
									.format('Ymd'),
							fhsjz : Ext.getCmp('fhsjz').getValue()
									.format('Ymd'),
