    ...
    // 子客户名称下拉
     ...
     var spxx = new Ext.form.ComboBox({
    	...
    	spxx.on('select', function(comboBox) {
				...
				// 调用ajax 从后台获取列名
				Ext.Ajax.request({
							url : 'XiaoShowDan!queryJC.action',
							params : 'spid=' + spid+'&khid='+khid,
							method : 'POST',
							success : function(resp, opts) {
								var res = Ext.util.JSON
										.decode(resp.responseText);
								Ext.getDom('jc').value = formatNambToDouble(res[0].JC);
								Ext.getDom('ysdh').value = res[0].YSDH;
							},
							failure : function() {
...
//										{
//											var firstValue = zkhCombo.getStore().getRange()[0].data.value;//这种方法可以获得第一项的值 
       ...
       var pymValue2 ="";      
        ...
        Ext.getCmp('zkhCombo').on('beforequery',function(e){     
       		 if(!e.forceAll){   
            		pymValue2 = e.query;   
            		//if (!pymValue == "")
            		Ext.getCmp('zkhCombo').getStore().reload({callback: function(records, options, success)
									{
...
function submitData()
{
	if(baseCheck())
	{
		...
		Ext.Ajax.request
		({    
			url:'XiaoShowDan!savaData.action', 
...
function baseCheck()
{
	...
	if(Ext.getCmp('khCombo').getValue()=="%" || Ext.getCmp('khCombo').getValue()=="")
	{
...
function fzData(bj)
{
	...
	record.set('ysdh'    ,Ext.getDom('ysdh').value);
	
	if(bj=="save")
	{
		record.set('khbm'	,Ext.getCmp('khCombo').getValue());
		record.set('zkhbm'	,Ext.getCmp('zkhCombo').getValue());
...
function jsje1(obj)
{
	...
	{	
		...
		var ysdh = Ext.getDom('ysdh').value;
		
		var jc = formatNambToDouble(Ext.getDom('jc').value);

		if(ysdh!="" && parseFloat(sl) > parseFloat(jc) ){
			Ext.Msg.alert('错误','销售数量不能大于预售结存量！！！');
			return;
		}
		if(dj!="")
		{
...
function printTT()
{
	...
	{
		var data = fzData("");
		var url = 'XiaoShowDanPrint!PrintData.action?data='+encodeURIComponent(data);
		openMaxWindow(url);
	}
}
...
function printTable(flag)
{
	...
	{
		var data = fzData("");
		var url = 'XiaoShowDanPrint!PrintData.action?flag='+flag+'&data='+encodeURIComponent(data);
		openMaxWindow(url);
	}
}
