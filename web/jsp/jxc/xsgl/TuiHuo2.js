		...
		store:new Ext.data.Store({
    						autoLoad:false,
							 proxy: new Ext.data.HttpProxy({
							 						url:'ComboboxAction!queryJhdh.action?dhlx=2'}),
							reader: new Ext.data.JsonReader({
							  root:'combobox',
   							fields:['value','text']}), 
						remoteSort: true  }),
   		...
   		jhdhCombo.getStore().reload({callback: function(records, options, success)
										{
											...
											{
												Ext.MessageBox.alert('消息!!!','没有对应的发货单号，请重新选择！！！');
											}
										}
									})
    }
  ...
  jhdhCombo.getStore().on('beforeload', function() {
   ...
   // 品种规格名称下拉
    ...
    var splbRecord = Ext.data.Record.create([
        ...
        //{name: 'dj'			,mapping:'dj'     		,type: 'string'},
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy
        					({url:'queryFhdxx!queryXsdxx.action',
    ...
    var columns = new Ext.grid.ColumnModel([  
	    ...
	    {header: '发货数量' 		,dataIndex: 'jhsl'},
	    //{header: '余数' 			,dataIndex: 'ys'},
...
function tuiHuo()
{
	...
	Ext.Ajax.request
	({    
		url:'TuiHuo2!saveTuiHuoxx.action', 
